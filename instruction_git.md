...
<Уточнить информацию про --all на семинаре и потом дописать>
...
4. Если в нашем проекте несколько веток, бывает удобно получить информацию об истории изменений в графическом представлении, чтобы увидеть, в какой ветке какие изменения производились. 

   Для этого в команде __*git log*__ предусмотрена опция __*--graph*__: 

        git log --graph

Ее удобно использовать в сочетании с опцией __*--oneline*__, чтобы результат не был слишком громоздким: 
...
Цепочка последовательных коммитов образует ветку. Ветка - это история изменений нашего репозитория. 

Каждая ветка иммеет свое название. Корневая (главная) ветка по умолчанию носит название __*master*__ или *__main__*. От нее могут отпочковываться и потом вливаться в нее обратно другие, второстепенные, ветки, название которым придумывает тот, кто их создает. В названии ветки желательно отражать ее назначение. 

#### *Переименование главной ветки*

Главную ветку проекта можно переименовать при помощи команды

        git config —global init.defaultBranch <Новое_имя>
...
Переименовать вновь созданную ветку можно с помощью команды 

        git branch -m <новое_имя_ветки>

### __*Слияние веток*__

Если мы завершили написание фрагмента текста или кода, относящегося к данной ветки, и полностью им довольны, то, после того как сделан завершающий коммит, можно перенести его в "чистовик" - слить с родительской  веткой. 

Чтобы сделать это, надо: 

1. Перейти в родительскую ветку: 
        
        git checkout <имя_род_ветки>

2. Оттуда вызвать команду

        git merge <имя_вливаемой_ветки>

### __*Конфликты*__

Конфликт происходит в том случае, если информация в некотором фрагменте вливаемой ветки отличается от информации в этом же фрагменте родительской ветки. 

В таком случае команда __*git merge*__ выдаст ошибку и предложит нам на выбор варианты дальнейших действий: 

   * Оставить в том виде, в котором фрагмент был в родительской ветке
   * Принять вариант из вливаемой ветки
   * Принять оба варианта и отредактировать результат вручную
   * сравнить варианты

Дальнейшая работа над фрагментом зависит от того, какой вариант был вами выбран. 

### __*Удаление веток*__

Если некоторая ветка выполнила свое предназначение и была уже слита с родительской веткой, ее можно удалить. Для этого используется команда: 

        git branch -d <имя_удаляемой_ветки>

Данная команда должна вызываться из родительской ветки, куда перед этим нужно перейти. 

**ВАЖНО:** Буква __"-d"__ в данной командре должна быть обязательно маленькой, если мы не хотим "нарваться" на ошибку. С таким флагом команда удаляет только те ветки, которые уже были слиты нами с родительской. Использование флага __"-D"__ приведет к удалению любой названной ветки независимо от того, применялась к ней уже команда __*git merge*__ или еще нет.  
